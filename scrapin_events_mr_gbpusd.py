...
MR_SYMBOL: str = "GBP-USD"
MR_SAMPLE_INTERVAL_SECS: int = 200
MR_LOOKBACK: int = 7
MR_THRESH: float = 1.050
MR_EPS: float = 0.0
MR_TRADE_SIZE_USD: float = 40_000.0
MR_SHOCK_BPS: float = 50.0
MR_COOLDOWN_SAMPLES: int = 5
MR_SEND_ORDERS: bool = True
MR_FX_MIN_ORDER_GBP: float = 25_000.0
MR_FX_ORDER_INCREMENT_GBP: float = 1_000.0
MR_SIGNAL_LOG_PATH: Path = ROOT / "ALL_DATA" / "strategy_events" / "gbp_usd_mr_signals.csv"
MR_STATE_PATH: Path = ROOT / "ALL_DATA" / "strategy_events" / "gbp_usd_mr_state.json"
...
class MeanReversionSignal:
    def __init__(
        ...
        self.mid_history: deque[float] = deque(maxlen=lookback)
        self.last_sample_ts: datetime | None = None
        self.latest_mid: float | None = None
        self.latest_mid_ts: datetime | None = None
        self.last_sample_mid: float | None = None
        self.cooldown_remaining: int = 0
        self.signal_position: int = 0
        self.load_state()
    ...
    def load_state(self) -> None:
        ...
        try:
            ...
            print("MR state load failed", exc)
            return
        ...
        for mid in payload.get("mid_history", [])[-self.lookback:]:
            try:
                self.mid_history.append(float(mid))
            except (TypeError, ValueError):
                pass

        self.last_sample_ts = self.parse_state_ts(payload.get("last_sample_ts"))
        self.latest_mid_ts = self.parse_state_ts(payload.get("latest_mid_ts"))
        self.latest_mid = self.safe_state_float(payload.get("latest_mid"))
        self.last_sample_mid = self.safe_state_float(payload.get("last_sample_mid"))
        self.cooldown_remaining = int(payload.get("cooldown_remaining", 0) or 0)
        self.signal_position = int(payload.get("signal_position", 0) or 0)
        print("MR state loaded", payload)
    ...
    def save_state(self) -> None:
        ...
        payload = {
            ...
            "last_sample_ts": self.last_sample_ts.isoformat() if self.last_sample_ts is not None else None,
            "latest_mid": self.latest_mid,
            "latest_mid_ts": self.latest_mid_ts.isoformat() if self.latest_mid_ts is not None else None,
            "last_sample_mid": self.last_sample_mid,
    ...
    @staticmethod
    def parse_state_ts(value: Any) -> datetime | None:
        if not value:
            return None
        try:
    ...
    @staticmethod
    def safe_state_float(value: Any) -> float | None:
        try:
            return float(value)
        except (TypeError, ValueError):
            return None

    def update_mid(self, symbol: str, ts: datetime, bid: float | None, ask: float | None) -> None:
        if symbol != self.symbol or bid is None or ask is None:
            return
        self.latest_mid = (bid + ask) / 2.0
        self.latest_mid_ts = ts
        self.save_state()

    def sample_due(self, ts: datetime) -> dict[str, Any] | None:
        if self.latest_mid is None:
            return None
        if self.last_sample_ts is not None:
            elapsed = (ts - self.last_sample_ts).total_seconds()
            if elapsed < self.sample_interval_secs:
                return None
        ...
        sample_return_bps = np.nan
        if self.last_sample_mid is not None and self.last_sample_mid > 0.0:
            sample_return_bps = (mid / self.last_sample_mid - 1.0) * 10_000.0
            if abs(sample_return_bps) > self.shock_bps:
                self.cooldown_remaining = self.cooldown_samples
        self.last_sample_mid = mid

        if len(self.mid_history) < self.lookback:
            event = self.write_signal(ts, mid, np.nan, self.signal_position, "warming_up")
            if self.cooldown_remaining > 0:
                self.cooldown_remaining -= 1
            self.save_state()
            return event

        values = np.asarray(self.mid_history, dtype=np.float64)
        sd = values.std(ddof=1)
        z = (values[-1] - values.mean()) / sd if sd > 0.0 else np.nan

        target_position = self.signal_position
        if np.isfinite(z):
            if z > self.thresh:
                target_position = -1
            elif z < -self.thresh:
        ...
        if self.cooldown_remaining > 0:
            ...
            reason = "hold" if target_position == self.signal_position else "signal_change"

        prior_position = self.signal_position
        self.signal_position = target_position
        event = self.write_signal(
            ts,
            mid,
            z,
    ...
    def write_signal(
        ...
        sample_return_bps: float = np.nan,
    ) -> dict[str, Any]:
        prior = self.signal_position if prior_position is None else prior_position
        trade_notional_usd = (signal_position - prior) * self.trade_size_usd
        ...
        event = {
            ...
            "cooldown_remaining": self.cooldown_remaining,
            "mid": mid,
            "sample_return_bps": sample_return_bps,
            "z": z,
        ...
        print("MR signal", row.iloc[0].to_dict())
        return event
...
class EventStore:
    ...
    def product_session_path(self, symbol: str, file_date: str) -> Path:
        ...
        return self.product_dir(symbol) / file_date / filename
    ...
    def flush_symbol(self, symbol: str, force: bool = False) -> None:
        ...
        print("Flushed", len(df), symbol, f"({raw_count - len(df)} duplicate timestamp rows dropped)")
        rows.clear()
...
class IBApp(EWrapper, EClient):
    ...
    def __init__(self, event_store: EventStore) -> None:
        ...
        self.mr_signal = MeanReversionSignal(
            symbol=MR_SYMBOL,
            sample_interval_secs=MR_SAMPLE_INTERVAL_SECS,
            lookback=MR_LOOKBACK,
            thresh=MR_THRESH,
            eps=MR_EPS,
            trade_size_usd=MR_TRADE_SIZE_USD,
            shock_bps=MR_SHOCK_BPS,
            cooldown_samples=MR_COOLDOWN_SAMPLES,
            log_path=MR_SIGNAL_LOG_PATH,
            state_path=MR_STATE_PATH,
        )
    ...
    def execute_mr_signal_order(self, signal_event: dict[str, Any] | None) -> None:
        if not MR_SEND_ORDERS or signal_event is None:
            return

        trade_notional_usd = float(signal_event.get("trade_notional_usd") or 0.0)
        mid = float(signal_event.get("mid") or 0.0)
        if trade_notional_usd == 0.0 or mid <= 0.0:
            return

        quote = next(
            (
                q for q in self.quotes.values()
                if q.get("output_symbol") == MR_SYMBOL and q.get("trade_contract") is not None
            ),
            None,
        )
        if quote is None:
            print("MR order skipped: no GBP-USD trade contract")
            return

        if self.next_order_id is None:
            print("MR order skipped: no IB order id yet")
            return

        contract: Contract = quote["trade_contract"]
        action = "BUY" if trade_notional_usd > 0.0 else "SELL"
        raw_quantity_gbp = abs(trade_notional_usd) / mid
        quantity_gbp = round(raw_quantity_gbp / MR_FX_ORDER_INCREMENT_GBP) * MR_FX_ORDER_INCREMENT_GBP
        if quantity_gbp < MR_FX_MIN_ORDER_GBP:
            print(
                "MR order skipped: rounded GBP quantity below minimum",
                {
                    "raw_quantity_gbp": raw_quantity_gbp,
                    "rounded_quantity_gbp": quantity_gbp,
                    "min_order_gbp": MR_FX_MIN_ORDER_GBP,
                },
            )
            return
        ...
        order.action = action
        order.orderType = "MKT"
        order.totalQuantity = int(quantity_gbp)
        order.tif = "DAY"
        ...
        print(
            "MR placing order",
            {
                "order_id": order_id,
                "action": action,
                "quantity_gbp": order.totalQuantity,
                "trade_notional_usd": trade_notional_usd,
                "mid": mid,
                "signal_position": signal_event.get("signal_position"),
                ...
                while True:
                    ...
                    if app.connected_flag:
                        ...
                        try:
                            ...
                            print("Persist base quotes failed", exc)
                            traceback.print_exc()

                        try:
                            signal_event = app.mr_signal.sample_due(datetime.now(ET))
                            app.execute_mr_signal_order(signal_event)
                        except Exception as exc:
                            print("MR strategy loop failed", exc)
                            traceback.print_exc()

                        try:
                            event_store.flush_due()
                        except Exception as exc:
                            print("Event store flush failed", exc)
                            traceback.print_exc()
                        ...
                        if now - last_check >= 60:
                            try:
                                ...
                                print("Equity resubscribe check failed", exc)
                                traceback.print_exc()
                            last_check = now
